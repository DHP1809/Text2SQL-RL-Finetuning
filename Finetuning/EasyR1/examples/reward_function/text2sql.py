...
def normalize_results(results: list) -> list:
    ...
    for row in results:
        normalized_row = tuple(
            sorted(str(val).strip().lower() if val is not None else ''
            for val in row)
        )
...
def execute_sql(sql_query: str, conn: sqlite3.Connection, timeout: int = 10) -> Tuple[bool, list]:
...
def execution_reward(response_sql: str, gt_answer: str, db_path: str) -> float:
    ...
    try:
        ...
        gen_success, gen_results = execute_sql(opt_gen_sql, conn, timeout)
        #print(gen_results)
        if not gen_success: return 0.0 # GT lỗi thì bỏ qua
